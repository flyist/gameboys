...
public class Bot extends ListenerAdapter {
    ...
    private String runCommands(GenericMessageEvent event, String command) {
        if(command.equalsIgnoreCase("!raffle") && mods.contains(event.getUser().getNick())) {
            try {
                ...
                JSONObject json = new JSONObject(JSONParser.readUrl(String.format("https://tmi.twitch.tv/group/user/%s/chatters", GameBoys.CHANNEL)));
    ...
    public void onPing(PingEvent event) throws Exception {
        GameBoys.bot.sendRaw().rawLineNow(String.format("PONG %s\r\n", event.getPingValue()));
    }

    private void sendMessage(String message) {
        GameBoys.bot.sendIRC().message("#" + GameBoys.CHANNEL, message);
    }
}